...
bart_sample_general<-function(T_level, X, Y_obs){
  ...
  n =length(T_level)
  if (length(Y_obs)!= n){
    print("check dimension of the treatment and the outcome")
  }
  if (dim(X)[1]!= n){
    print("check dimension of the confounders and the outcome")
  }
...
BCF_sample_general<-function(T_level, X, Y_obs){
...
CART_general<-function(T_level, X, Y_obs, tau){
  ...
  if (length(tau_estimated)!= n){
    print("check dimension of the causal effect (tau)")
  }
